...
const ExportOptions = ({ onSingleDownload, onBulkGenerate, hasCSV, onGenerateQRCodes, hasSelectedQRs }) => {
  ...
  return (
    <div className="glass-panel p-4">
      <h3 className="text-lg font-medium text-white/60 mb-4">📤 Export</h3>
      <div className="space-y-4">
        ...
        <div className="bg-white/5 rounded-xl p-3 mt-3"><p className="text-sm font-medium mb-2">QR Codes</p><div className="flex gap-2"><button onClick={() => onGenerateQRCodes()} disabled={!hasCSV || !hasSelectedQRs} className="btn-secondary flex-1 animate-pulse hover:animate-none transition">Generate QR PNGs ZIP</button></div><p className="text-xs text-gray-400 mt-2">Generates QR PNGs locally for each CSV row and selected QR fields. Then upload the ZIP and align QR images like signatures.</p></div>
      </div>
    </div>
  );
};
